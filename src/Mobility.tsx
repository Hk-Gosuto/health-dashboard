...
export default function Mobility({ dailyMobility, cutoffDate }: Props) {
  ...
  if (!hasData) {
    return <div className="text-zinc-500 text-center py-20">{tText('No mobility data found.')}</div>
  }

  return (
    <div className="space-y-6">
      <TabHeader title={tText('Mobility & Gait')} description={tText('Walking speed, step length, gait symmetry, stair climbing, and balance metrics — key indicators of functional fitness and aging.')} />

      {/* Summary */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
        {avgSpeed !== null && <StatBox label={tText('Walking Speed')} value={fmt(avgSpeed, 2)} unit="km/h" color={COLORS.blue} sub={tText('30d avg')} />}
        {avgStepLen !== null && <StatBox label={tText('Step Length')} value={fmt(avgStepLen, 1)} unit="cm" color={COLORS.green} sub={tText('30d avg')} />}
        {avgDoubleSupport !== null && <StatBox label={tText('Double Support')} value={fmt(avgDoubleSupport, 1)} unit="%" color={COLORS.orange} sub={tText('30d avg')} />}
        {avgAsymmetry !== null && <StatBox label={tText('Asymmetry')} value={fmt(avgAsymmetry, 1)} unit="%" color={avgAsymmetry < 10 ? COLORS.green : COLORS.red} sub={avgAsymmetry < 10 ? tText('Normal') : tText('Elevated')} />}
        {totalFlights > 0 && <StatBox label={tText('Flights')} value={`${totalFlights}`} sub={tText('Last 30d')} color={COLORS.cyan} />}
        {latestSteadiness !== null && <StatBox label={tText('Steadiness')} value={fmt(latestSteadiness, 0)} unit="%" color={latestSteadiness >= 80 ? COLORS.green : COLORS.orange} sub={tText('Latest')} />}
        {latestSixMin !== null && <StatBox label={tText('6-Min Walk')} value={`${latestSixMin}`} unit="m" color={COLORS.purple} sub={tText('Latest')} />}
      </div>
      ...
      {weeklySpeed.length > 1 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
          <div className="flex items-start justify-between mb-1">
            <div>
              <h3 className="text-sm font-medium text-zinc-300">{tText('Walking Speed')}</h3>
              <p className="text-xs text-zinc-500 mt-0.5">{tText('Weekly average. Higher walking speed is associated with better overall health and longevity.')}</p>
            </div>
            <AISummaryButton title={tText('Walking Speed')} description={tText('Weekly average walking speed')} chartData={weeklySpeed} />
          </div>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
              <AreaChart margin={chartMargin} data={weeklySpeed}>
                ...
                <Tooltip content={<ChartTooltip formatter={(v) => [`${v} km/h`, tText('Walking Speed')]} />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {weeklyStepLen.length > 1 && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Step Length')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText('Longer strides indicate better mobility and leg strength.')}</p>
              </div>
              <AISummaryButton title={tText('Step Length')} description={tText('Weekly average step length')} chartData={weeklyStepLen} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
                <AreaChart margin={chartMargin} data={weeklyStepLen}>
                  ...
                  <XAxis dataKey="week" tick={{ fontSize: 10, fill: ct.tick }} tickFormatter={shortDate} />
                  <YAxis domain={['auto', 'auto']} tick={{ fontSize: 10, fill: ct.tick }} />
                  <Tooltip content={<ChartTooltip formatter={(v) => [`${v} cm`, tText('Step Length')]} />} />
        ...
        {weeklyDoubleSupport.length > 1 && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Double Support Time')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText('% of walking time with both feet on ground. Lower = better balance and confidence.')}</p>
              </div>
              <AISummaryButton title={tText('Double Support Time')} description={tText('% of walking with both feet down')} chartData={weeklyDoubleSupport} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
                <AreaChart margin={chartMargin} data={weeklyDoubleSupport}>
                  ...
                  <Tooltip content={<ChartTooltip formatter={(v) => [`${v}%`, tText('Double Support')]} />} />
        ...
        {weeklyAsymmetry.length > 1 && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Walking Asymmetry')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText('Left/right leg imbalance. Under 10% is normal; higher may indicate injury risk or compensation.')}</p>
              </div>
              <AISummaryButton title={tText('Walking Asymmetry')} description={tText('Left/right leg imbalance %')} chartData={weeklyAsymmetry} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
                <AreaChart margin={chartMargin} data={weeklyAsymmetry}>
                  ...
                  <Tooltip content={<ChartTooltip formatter={(v) => [`${v}%`, tText('Asymmetry')]} />} />
        ...
        {weeklyFlights.length > 1 && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Flights Climbed')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText('Weekly total floors. Stair climbing is excellent for lower body strength and cardio.')}</p>
              </div>
              <AISummaryButton title={tText('Flights Climbed')} description={tText('Weekly total floors climbed')} chartData={weeklyFlights} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
                <AreaChart margin={chartMargin} data={weeklyFlights}>
                  ...
                  <Tooltip content={<ChartTooltip formatter={(v) => [`${v} flights`, tText('Flights')]} />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {(weeklyStairAscent.length > 1 || weeklyStairDescent.length > 1) && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Stair Speed')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText('How quickly you climb and descend stairs. An important functional mobility metric.')}</p>
              </div>
              <AISummaryButton title={tText('Stair Speed')} description={tText('Ascent and descent speed')} chartData={weeklyStairAscent.length > 0 ? weeklyStairAscent : weeklyStairDescent} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
                <AreaChart margin={chartMargin} data={weeklyStairAscent.length >= weeklyStairDescent.length ? weeklyStairAscent.map((d, i) => ({
                  ...
                  <XAxis dataKey="week" tick={{ fontSize: 10, fill: ct.tick }} tickFormatter={shortDate} />
                  <YAxis domain={['auto', 'auto']} tick={{ fontSize: 10, fill: ct.tick }} />
                  <Tooltip content={<ChartTooltip formatter={(v, name) => [`${v} m/s`, name === 'ascent' ? tText('Ascent') : tText('Descent')]} />} />
        ...
        {steadinessData.length > 1 && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Walking Steadiness')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText("Apple's fall-risk assessment. Higher is better. Below 60% is flagged as low.")}</p>
              </div>
              <AISummaryButton title={tText('Walking Steadiness')} description={tText('Fall-risk assessment score')} chartData={steadinessData} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
                <ScatterChart margin={chartMargin}>
                  ...
                  <ZAxis range={[30, 50]} />
                  <Tooltip content={<ChartTooltip formatter={(v) => [`${v}%`, tText('Steadiness')]} />} />
        ...
        {sixMinData.length > 1 && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Six-Minute Walk Test')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText('Clinical fitness test. 400-700m is typical for healthy adults. Higher = better endurance.')}</p>
              </div>
              <AISummaryButton title={tText('Six-Minute Walk Test')} description={tText('Distance walked in 6 minutes')} chartData={sixMinData} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
                <ScatterChart margin={chartMargin}>
                  ...
                  <YAxis domain={['auto', 'auto']} tick={{ fontSize: 10, fill: ct.tick }} />
                  <ZAxis range={[40, 60]} />
                  <Tooltip content={<ChartTooltip formatter={(v) => [`${v} m`, tText('Distance')]} />} />
