...
export default function Daylight({ dailyDaylight, cutoffDate, granularity: _granularity }: Props) {
  ...
  if (filtered.length === 0) {
    return <div className="text-zinc-500 text-center py-20">{tText('No daylight data found.')}</div>
  }

  return (
    <div className="space-y-6">
      <TabHeader title={tText('Daylight')} description={tText('Time in daylight, daily exposure patterns, and seasonal light habits.')} />
      {/* Summary */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <StatBox
          label={tText('Daily Average')}
        ...
        />
        <StatBox
          label={tText('Days < 20 min')}
          value={`${daysBelow20}`}
          sub={`of last ${recent.length} days`}
          color={daysBelow20 > 15 ? '#ef4444' : '#f97316'}
        />
        <StatBox
          label={tText('Days > 1 hr')}
          value={`${daysAbove60}`}
          sub={`of last ${recent.length} days`}
          color="#22c55e"
        />
        <StatBox label={tText('Best Day')} value={`${maxDay}`} unit="min" sub={tText('Last 30 days')} />
        <StatBox label={tText('Total Days')} value={`${totalDays}`} sub={tText('With daylight data')} />
        <StatBox
          label={tText('Goal')}
          value="30+"
          unit="min"
          sub={tText('Recommended daily')}
          color="#71717a"
        />
      </div>
      ...
      {weeklyData.length > 1 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
          <div className="flex items-start justify-between mb-1">
            <div>
              <h3 className="text-sm font-medium text-zinc-300">{tText('Daily Daylight Exposure (weekly avg)')}</h3>
              <p className="text-xs text-zinc-500 mt-0.5">{tText('Minutes of outdoor light detected by Apple Watch. 30+ min/day supports circadian rhythm, vitamin D, and mood.')}</p>
            </div>
            <AISummaryButton title={tText('Daily Daylight Exposure (weekly avg)')} description={tText('Minutes of outdoor light detected by Apple Watch. 30+ min/day supports circadian rhythm, vitamin D, and mood.')} chartData={weeklyData} />
          </div>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
              <AreaChart margin={{ top: 5, right: 5, bottom: 0, left: -15 }} data={weeklyData}>
                ...
                <ReferenceLine y={30} stroke="#71717a" strokeDasharray="3 3" label={{ value: tText('30 min goal'), position: 'right', fill: ct.tick, fontSize: 10 }} />
                <Tooltip content={<ChartTooltip formatter={(v) => [`${v} min`, tText('Daylight')]} />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {monthlyPattern.length > 6 && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Seasonal Pattern')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText('Average daily daylight by month (all years combined)')}</p>
              </div>
              <AISummaryButton title={tText('Seasonal Pattern')} description={tText('Average daily daylight by month (all years combined)')} chartData={monthlyPattern} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
                <BarChart margin={{ top: 5, right: 5, bottom: 0, left: -15 }} data={monthlyPattern}>
                  ...
                  <Tooltip content={<ChartTooltip formatter={(v) => [`${v} min/day`, tText('Avg Daylight')]} />} />
        ...
        {monthlyByYear.length > 2 && (
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
            <div className="flex items-start justify-between mb-1">
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{tText('Monthly Average Over Time')}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{tText('Avg daily minutes per month — shows year-over-year trends')}</p>
              </div>
              <AISummaryButton title={tText('Monthly Average Over Time')} description={tText('Avg daily minutes per month — shows year-over-year trends')} chartData={monthlyByYear} />
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
