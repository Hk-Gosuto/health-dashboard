...
export default function MenstrualCycle({ menstrualRecords, wristTempRecords, cutoffDate }: Props) {
  ...
  if (!hasData) {
    return <div className="text-zinc-500 text-center py-20">{tText('No menstrual cycle data found.')}</div>
  }

  return (
    <div className="space-y-6">
      <TabHeader title={tText('Menstrual Cycle')} description={tText('Cycle length, period duration, flow patterns, and temperature tracking.')} />
      {/* Summary Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
        {avgCycleLength !== null && (
          <StatBox label={tText('Cycle Length')} value={`${avgCycleLength}`} unit="days" sub={tText('Average')} color={COLORS.purple} />
        )}
        {avgPeriodDays !== null && (
          <StatBox label={tText('Period')} value={fmt(avgPeriodDays, 1)} unit="days" sub={tText('Average')} color={CYCLE_COLORS.flow} />
        )}
        {shortestCycle !== null && longestCycle !== null && (
          <StatBox label={tText('Range')} value={`${shortestCycle}-${longestCycle}`} unit="days" sub={tText('Min-Max cycle')} />
        )}
        {regularity !== null && (
          <StatBox
            label={tText('Regularity')}
        ...
        <StatBox label={tText('Cycles')} value={`${cycles.length}`} sub={tText('Total detected')} />
        <StatBox label={tText('Flow Days')} value={`${totalFlowDays}`} sub={tText('Total tracked')} color={CYCLE_COLORS.flow} />
        {nextPeriodDate && (
          <StatBox label={tText('Next Period')} value={nextPeriodDate.substring(5)} sub={tText('Estimated')} color={COLORS.pink} />
        )}
      </div>
      ...
      {cycleCalendar.length > 0 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
          <h3 className="text-sm font-medium text-zinc-300 mb-1">{tText('Recent Cycles')}</h3>
          <p className="text-xs text-zinc-500 mb-3">{tText('Period (red), estimated fertile window (purple), estimated ovulation (green)')}</p>
          ...
          <div className="flex gap-4 mt-3">
            <div className="flex items-center gap-1.5 text-xs text-zinc-400"><div className="w-3 h-3 rounded-sm" style={{ backgroundColor: CYCLE_COLORS.flow }} /> {tText('Period')}</div>
            <div className="flex items-center gap-1.5 text-xs text-zinc-400"><div className="w-3 h-3 rounded-sm" style={{ backgroundColor: CYCLE_COLORS.fertile + '80' }} /> {tText('Fertile window')}</div>
            <div className="flex items-center gap-1.5 text-xs text-zinc-400"><div className="w-3 h-3 rounded-sm" style={{ backgroundColor: CYCLE_COLORS.ovulation }} /> {tText('Ovulation')}</div>
          </div>
        </div>
      )}
      ...
      {cycleLengthData.length > 1 && (
        <ChartCard title={tText('Cycle Length')} description={tText('Days between period starts. Normal range: 21-35 days.')} chartData={cycleLengthData}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {periodDurationData.length > 1 && (
          <ChartCard title={tText('Period Duration')} description={tText('Number of flow days per cycle')} chartData={periodDurationData}>
        ...
        {flowDistribution.length > 0 && (
          <ChartCard title={tText('Flow Intensity Distribution')} description={tText('Days by flow level across all tracked data')} chartData={flowDistribution}>
      ...
      {bbtData.length > 1 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
          <div className="flex items-start justify-between mb-1">
            <div>
              <h3 className="text-sm font-medium text-zinc-300">{tText('Temperature Tracking')}</h3>
            ...
            <AISummaryButton title={tText('Temperature Tracking')} description={tText('Basal body or wrist temperature over time')} chartData={bbtData} />
          </div>
          <div className="h-56">
      ...
      {flowTimeline.length > 0 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4">
          <div className="flex items-start justify-between mb-1">
            <div>
              <h3 className="text-sm font-medium text-zinc-300">{tText('Flow Timeline')}</h3>
              <p className="text-xs text-zinc-500 mt-0.5">{tText('Daily flow intensity over time')}</p>
            </div>
            <AISummaryButton title={tText('Flow Timeline')} description={tText('Daily flow intensity over time')} chartData={flowTimeline} />
          </div>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={1}>
              <ScatterChart margin={chartMargin}>
